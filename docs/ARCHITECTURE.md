...
Logs follow the same correlation model. `logback-spring.xml` ships structured JSON logs to Loki (`Loki4jAppender`) alongside the existing console and rolling-file appenders, so the trace/span IDs already in the log pattern carry through into Loki queries. `GlobalExceptionHandler` adds `http_status_code` and `error_message` to the MDC for the duration of building an error response, then removes them, so every error log line in that window is queryable by status code without leaking that context into logs from other requests sharing the same thread.
