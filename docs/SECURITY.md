...
2. **Roles are not enforced.** `JwtAuthenticationFilter` builds the authentication with an empty authority list, so the `USER`/`ADMIN` roles stored on `User` never reach the security context and no endpoint can currently be restricted to admins. Ownership checks (a user reading only their own payments) rely on service-layer logic rather than declarative rules.
