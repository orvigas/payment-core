...
- **Grafana** (`http://localhost:3000`, admin/admin by default) is the fastest way to look at all of this together: the **Payment Core - Overview** dashboard auto-loads on startup with no import step, covering every metric below plus a link out to Jaeger. See [docs/DEPLOYMENT.md](DEPLOYMENT.md#grafana-dashboard).
...
- **Jaeger** (`http://localhost:16686`) shows where time goes inside a request; use it before guessing at a bottleneck. Sampling is 100% locally, which is itself a cost — reduce `management.tracing.sampling.probability` in production. Its **Monitor** tab (`/monitor`) and the **Jaeger - Service Performance Monitor** Grafana dashboard both show request/error/duration aggregated by operation, derived from span content by `otel-collector` — see [docs/DEPLOYMENT.md](DEPLOYMENT.md#service-performance-monitor) for how that pipeline works.
